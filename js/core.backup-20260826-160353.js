...
(function () {
  ...
  function migrate(raw) {
    ...
    /* v3 → v4: multi-device sync through a shared folder. Every day record
       and every id-keyed row gains an `updatedAt`, which is what a merge
       compares — without it, two machines can only fall back to
       whole-file-newest-wins, and an evening where you logged a weight on one
       and a lab result on the other loses whichever wrote first.

       Additive only: nothing is removed or renamed, so a v4 save opened by an
       older build still loads through the deep-merge above.

       Backfill is deliberately conservative — see syncmerge.js — so nothing
       written before the upgrade can outrank a live edit after it. */
    if (!raw.version || raw.version < 4) {
      if (Hub.syncMerge) Hub.syncMerge.backfillStamps(s);
    }
  ...
  function heal(s) {
    ...
    ["vitals", "checkups", "meds", "injuries", "breathTests",
     "labs", "cycles", "customHabits", "photos", "vo2max"].forEach(function (k) {
  ...
  function load() {
    ...
    /* Seed the index from what was just loaded, so the first save after a
       reload doesn't stamp every record in your history as changed today. */
    if (Hub.syncMerge) syncIndex = Hub.syncMerge.index(STATE);
    return STATE;
  }

  /* What every syncable record looked like at the last save. Comparing
     against it is how a record gets an honest `updatedAt` without every view
     in the app having to remember to set one — see syncmerge.js. */
  var syncIndex = null;

  var saveFailed = false;
  function save() {
    try {
      if (Hub.syncMerge) syncIndex = Hub.syncMerge.stampChanges(STATE, syncIndex);
  ...
  /* ----------------------------------------------------------------------
     LAZY CHART.JS
     Chart.js is 69KB over the wire and is used by exactly two views, but it
     was parsed on every cold start before the Dashboard could paint. Both
     consumers already bail cleanly when it's absent — `js/views/insights.js`
     guards `if (!canvas || !window.Chart)` and BASALT's `makeChart` opens
     with `if (!window.Chart) return null` — so the view renders immediately
     without charts and re-renders once the library lands.

     Only Chart.js is deferred. `fitness/basalt.js` registers 12 plain
     DOMContentLoaded listeners (and muscles.js another 5) with no readyState
     guard, so loading it after that event would silently never boot it.
     Making it lazy is a boot refactor, not a script-tag move.
     -------------------------------------------------------------------- */
  var SCRIPTS = {};
  function loadScript(src) {
    ...
    SCRIPTS[src] = new Promise(function (resolve, reject) {
      ...
      s.onerror = function () {
        delete SCRIPTS[src];                 // let a later attempt retry
  ...
  /* Views that draw with Chart.js. */
  var CHART_VIEWS = { insights: true, fitness: true };

  function ensureChart(name) {
    if (!CHART_VIEWS[name] || window.Chart) return;
    loadScript("vendor/chart.umd.min.js").then(function () {
      /* Only redraw if the user is still on a view that wants charts —
         they may have navigated on while it downloaded. */
      if (!CHART_VIEWS[active]) return;
      renderView(active);
      if (window.App && typeof window.App.refresh === "function") window.App.refresh();
    }).catch(function () {
      /* Offline and not yet cached. The guards mean the views simply render
         without charts rather than breaking. */
    });
  }
  ...
  var Hub = window.Hub = {
    ...
    setState: function (s) {
      STATE = migrate(s);
      /* Reseed, or the next save stamps every record in the incoming state as
         having changed on this machine right now — which would make a merged
         copy outrank the machine it came from. */
      if (Hub.syncMerge) syncIndex = Hub.syncMerge.index(STATE);
    },
