...
(function () {
    ...
    var cards = Object.keys(GOALS).map(function (id) {
      ...
      return '<button class="run-goal" data-rungoal="' + id + '" type="button">' +
        ...
        '<div class="run-goal__meta">' +
          ...
          '<span class="run-goal__chip">starts from zero</span>' +
        '</div>' +
      '</button>';
    }).join("");
  ...
  function kindLabel(kind) {
    return ({ easy: "Easy", tempo: "Tempo", long: "Long", interval: "Intervals", sprint: "Sprint", walkrun: "Walk/Run", rest: "Rest" })[kind] || cap(kind);
  }
